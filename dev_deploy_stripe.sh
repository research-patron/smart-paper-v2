...
gcloud functions deploy create_stripe_checkout \
  ...
  --no-gen2 \
  --set-env-vars=GOOGLE_CLOUD_PROJECT=${PROJECT_ID},STRIPE_SUCCESS_URL=http://localhost:3000/subscription?success=true,STRIPE_CANCEL_URL=http://localhost:3000/subscription?canceled=true
...
gcloud functions deploy update_payment_method \
  ...
  --allow-unauthenticated \
  --no-gen2 \
  --set-env-vars=GOOGLE_CLOUD_PROJECT=${PROJECT_ID},STRIPE_RETURN_URL=http://localhost:3000/subscription
